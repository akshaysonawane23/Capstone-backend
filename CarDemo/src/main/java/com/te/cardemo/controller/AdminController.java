...
import com.te.cardemo.model.AdminRequest;
import com.te.cardemo.model.AdminResponse;
import com.te.cardemo.model.CarDetailsResponse;
import com.te.cardemo.service.AdminService;
...
public class AdminController {
		...
		if (adminService.checkIfUsernameExists(admin.getUsername())) {
			try {
				...
				return new ResponseEntity<AdminResponse>(
						new AdminResponse(true, "Invalid Username or Password", null, null), HttpStatus.BAD_REQUEST);
			}
			...
			return ResponseEntity
					.ok(new AdminResponse(false, "Authentication Success", jwtToken, adminDetails.getRole()));
		} else {
			return new ResponseEntity<AdminResponse>(
					new AdminResponse(true, "Username not Found, Please Signup", null, null), HttpStatus.BAD_REQUEST);
		}
	}
	...
	public ResponseEntity<?> createSignupAuthenticationToken(@RequestBody AdminDetails adminDetails) {
		...
		try {
			...
			return new ResponseEntity<AdminResponse>(
					new AdminResponse(true, "Username Already Exists, Please Login", null, null),
	...
	public ResponseEntity<?> getAllCarDetails(HttpServletRequest request) {
		...
		try {
			...
			return new ResponseEntity<CarDetailsResponse>(new CarDetailsResponse(true, "failure", null),
					HttpStatus.BAD_REQUEST);
		}
	}
	...
	public ResponseEntity<?> addCarDetails(@RequestBody CarDetails carDetails, HttpServletRequest request) {
		...
		try {
			...
			return new ResponseEntity<CarDetailsResponse>(
					new CarDetailsResponse(true, "Opps something went wrong", null), HttpStatus.BAD_REQUEST);
		}
	}
